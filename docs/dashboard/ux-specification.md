...
ARTIFACT SHAPE: `docs/api/openapi.yaml` (ParsedArtifact, SummaryArtifact, ImageArtifact)
...
> rendered as fully processed. The filing view is now a **rendered canonical hierarchy** covering
> the whole document, and coverage is displayed as five separate layers. The footnote index
> survives as one specialized view, unchanged in its guarantees.

---

# THE BETA UI — AUTHORITATIVE. Everything below this section is historical.

NOT IMPLEMENTED. No frontend code exists and none is generated by this document.
...
|  vertical, left     |                                                  |
|                     |  Original source | Parsed filing | Summary       |
|  Entity typeahead   |  Models and cost | Deep Dive                     |
|  Timeframe          |                                                  |
|  Parsing model      |  Live progress while a job runs                  |
|  Image model        |                                                  |
|  Summary model      |                                                  |
|  Analysis model     |                                                  |
|  [   Search   ]     |                                                  |
...
The left panel is **persistent** and collapses via an arrow control. When collapsed, a hamburger
control reopens it. The dashboard expands to fill the space either way.

## The search panel

**Entity typeahead.** Matches current name, former names, SEC filer name, current ticker,
historical ticker and alias. Served from the MINIMUM BETA CATALOG delivered in Phase 3 — this
view depends on Phase 3, never on the Phase 8 universe-scale expansion. An entity with no qualifying filing never appears — showing a symbol
the product cannot serve is worse than showing nothing. Results display the authoritative name
resolved from the CIK; a sampling label is never displayed as identity.

**Timeframe.** Bounded below by the entity's earliest known qualifying filing, which the Phase 3
catalog records, so the control
cannot offer a range the product cannot serve. Fiscal years, never calendar years.

**Four independent model selectors** — parsing, image, summary, analysis/chat. Each is populated
from the live-discovered catalog and each is chosen separately. None defaults from another.

When the selected **parsing model is multimodal**, the image selector stays **visible but
disabled**, with a one-line explanation that the parsing model handles images itself. It is not
hidden: hiding it would make the four-role model look like three.

**Search button.** Semi-transparent light blue. Enabled only when the entity, timeframe and all
required model roles are valid, the pairing is compatible, and the cost preview is inside budget.

## Visual design

Light mode. Light and dark grey surfaces. Dark grey menus and tables. Soft translucent blue and
green accents. The Search button is the single strongest accent on the page.

## Dashboard views

| View | Shows |
|---|---|
| **Original source** | The preserved SEC artifact, exactly as filed, with its hash |
| **Parsed filing** | The accepted parse, rendered from its filing-native labels |
| **Summary** | The separate summary artifact, with links back to source |
| **Models and cost** | Which model produced what, tokens, latency, cost, prompt version |
| **Deep Dive** | Scoped chat over the entity and timeframe |

Three distinct artifacts, three distinct views. A summary never silently stands in for a parse, and
a parse never silently stands in for the source.
...
**Unresolved content is displayed as unresolved.** A filing with an unresolved range shows
`PARTIAL` or `REVIEW_REQUIRED`. It is never rounded up to complete.

## Live progress

While a job runs the dashboard streams progress. Reconnection resumes rather than restarting.
**Opening a completed result invokes no model at all** — it is served from stored artifacts.
...
BUDGET EXCEEDED                 the job would exceed the authorized ceiling
...
filing from the timeline leads to the **filing view** in section 6A, which is the product's main
reading surface.
...
**One specialized view over `FINANCIAL_STATEMENT_FOOTNOTE` units.** It survives the Sprint 4.1
correction unchanged, because footnotes keep their own dedicated coverage guarantee. It is no
